...
static void _swap_out(Pde* pgdir, u_int asid)
{
	 ...
	 for (int i = 0; i < 1024; i++)
	 {
		  ...
		  if (!(*pte_entryp & PTE_V))
			   continue;
		  if ((PTE_ADDR(*pte_entryp) >> PGSHIFT) == ppn)
		  {
			   *pte_entryp &= ~PTE_V;	// clear PTE_V
...
static void _swap_in_aux(Pte* pte, u_char* da, struct Page* pp)
{
	 Pte* pte_entryp;
	 u_long ppn = (u_long)da >> PGSHIFT;

	 for (int i = 0; i < 1024; i++)
	 {
		  pte_entryp = pte + i;
		  if ((*pte_entryp & PTE_SWP) && \
					!(*pte_entryp & PTE_V) && \
					((PTE_ADDR(*pte_entryp) >> PGSHIFT) == ppn))
		  {
			   *pte_entryp |= PTE_V;
			   *pte_entryp &= ~PTE_SWP;
			   *pte_entryp &= 0xfff;
