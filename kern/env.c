 ...
 * in the 'pgdir'. Use permission bits 'perm | PTE_V' for the entries.
 *
 * Pre-Condition:
...
void env_init(void)
{
	 ...
	 * 'ENV_FREE' and insert them into the 'env_free_list'. Make sure, after
	 * the insertion, the order of envs in the list should be the same as they
	 * are in the 'envs' array. */
	...
	map_segment(base_pgdir,
				...
				PADDR(pages),
				UPAGES,
				ROUND(npage * sizeof(struct Page), BY2PG),
				PTE_G);
	map_segment(base_pgdir,
				0,
				PADDR(envs),
				UENVS,
				ROUND(NENV * sizeof(struct Env), BY2PG),
				PTE_G);
}
