...
Pte _do_tlb_refill(u_long va, u_int asid)
{
	 ...
	 * 'pte' associated with the virtual address 'va' in the current address
	 * space 'cur_pgdir'.
	 *   While 'page_lookup' returns 'NULL', indicating that the 'pte' could
	 * not be found allocate a new page using 'passive_alloc' until
	...
	while (!page_lookup(cur_pgdir, va, &pte))
