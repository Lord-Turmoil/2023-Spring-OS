...
void vprintfmt(fmt_callback_t out, void* data, const char* fmt, va_list ap)
{
	...
	for (;;)
	{
		...
		out(data, fallback, fmt - fallback);	// will be invoked even if _ is 0
		...
		/*
		* Notice that, printf will discard bad format specifier.
		* e.g. printf("[%-12]") will output only "[]".
		*/
		...
		width = 0;
		while (*fmt && is_digit(*fmt))	// only accept decimal
		{
		...
		case 'D':
			...
			if (num < 0)	// print_num only accept positive number.
			{
				neg_flag = 1;
				num = -num;
			}
